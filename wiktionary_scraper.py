...
class WiktionaryScraper:
    ...
    def extract_pronunciation(self, html):
    ...
    def extract_definition(self, html):
        soup = BeautifulSoup(html, 'lxml')
    ...
    def extract_word_type(self, html):
        soup = BeautifulSoup(html, 'lxml')
        
        word_types = ['noun', 'verb', 'adjective', 'adverb', 'pronoun', 'preposition']
        word_types_te = ['నామవాచకం', 'క్రియ', 'విశేషణం', 'క్రియా విశేషణం', 'సర్వనామం', 'విభక్తి ప్రత్యయాలు']
    ...
    def scrape_word(self, word):
        ...
        entry = PhoneticEntry(
            word=word,
            telugu_script=word,  # Already there in the Telugu script
            ipa=ipa,
